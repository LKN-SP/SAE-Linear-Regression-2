...
const BestLineCanvas = document.getElementById('best_line_canvas')
const BestLineLayer = BestLineCanvas.getContext('2d')
...
const sse_min_label = document.getElementById('sse_min')
...
var intercept = 0
var best_slope = 0
var best_intercept = 0
var min_sse = 10000000
...
function update_predictions() {
	...
	for (let i = 0; i < data_points.length; i++) {
		prediction = Math.floor((data_points[i][0] * slope + intercept) * 100) / 100
...
function update_data_table() {
	...
	for (let i = 0; i < data_points.length; i++) {
		...
		error = Math.floor((data_points[i][1] - predictions[i]) * 100) / 100
		cell5.innerHTML = error
		error_sq = Math.floor((error * error) * 100) / 100
		cell6.innerHTML = error_sq
		
		sse_value = sse_value + error_sq
	}
}

function update_sse() {
	sse_label.innerHTML = Math.floor(sse_value * 100) / 100
}

function update_comment() {
	if (sse_value > last_sse) {
		comment.innerHTML = "Hooooold on! It is getting worse!"
	} else if (sse_value < last_sse) {
		comment.innerHTML = "Good! The SSE is decreasing!"
		
		if (sse_value < min_sse) {
			best_slope = slope
			best_intercept = intercept
			min_sse = Math.floor(sse_value * 100) / 100
		}
		sse_min_label.innerHTML = min_sse.toString() + " (min)"
	}
}
...
function update_best_regression_line() {
	canvas = BestLineLayer
	canvas.clearRect(0, 0, LineCanvas.width, LineCanvas.height)
	
	if (best_intercept > axis_y_scale) {
		axis_point1 = [(axis_y_scale - best_intercept) / best_slope, axis_y_scale]
	} else if (best_intercept < 0) {
		axis_point1 = [(0 - best_intercept) / best_slope, 0]
	} else {
		axis_point1 = [0, best_intercept]
	}
	
	if (best_slope * axis_x_scale + best_intercept > axis_y_scale) {
		axis_point2 = [(axis_y_scale - best_intercept) / best_slope, axis_y_scale]
	} else if (best_slope * axis_x_scale + best_intercept < 0) {
		axis_point2 = [(0 - best_intercept) / best_slope, 0]
	} else {
		axis_point2 = [axis_x_scale, best_slope * axis_x_scale + best_intercept]
	}
